        ...
        <Box component="form" onSubmit={handleSubmit}>
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={4}>
              ...
              <Card sx={{ bgcolor: formData.brandColor || '#1976d2', color: '#fff', p: 2, textAlign: 'center' }}>
                <CardContent>
                  {formData.logo ? (
                    <Avatar src={URL.createObjectURL(formData.logo)} sx={{ width: 64, height: 64, mx: 'auto', mb: 2 }} />
                  ) : (
                    <Avatar sx={{ width: 64, height: 64, mx: 'auto', mb: 2 }}>{formData.name ? formData.name[0] : '?'}</Avatar>
                  )}
                  <Typography variant="h6">{formData.name || 'Restaurant Name'}</Typography>
                  <Typography variant="body2">{formData.address || 'Address'}</Typography>
